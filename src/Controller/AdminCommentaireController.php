<?php

namespace App\Controller;

use App\Entity\AdminCommentaire;
use App\Form\AdminCommentaireType;
use App\Repository\AdminCommentaireRepository;
use Doctrine\ORM\EntityManagerInterface;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Attribute\Route;

#[Route('/admin/commentaire')]
final class AdminCommentaireController extends AbstractController
{
    #[Route(name: 'app_admin_commentaire_index', methods: ['GET'])]
    public function index(AdminCommentaireRepository $adminCommentaireRepository): Response
    {
        return $this->render('admin_commentaire/index.html.twig', [
            'admin_commentaires' => $adminCommentaireRepository->findAll(),
        ]);
    }

    #[Route('/new', name: 'app_admin_commentaire_new', methods: ['GET', 'POST'])]
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        $adminCommentaire = new AdminCommentaire();
        $form = $this->createForm(AdminCommentaireType::class, $adminCommentaire);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->persist($adminCommentaire);
            $entityManager->flush();

            return $this->redirectToRoute('app_admin_commentaire_index', [], Response::HTTP_SEE_OTHER);
        }

        return $this->render('admin_commentaire/new.html.twig', [
            'admin_commentaire' => $adminCommentaire,
            'form' => $form,
        ]);
    }

    #[Route('/{id}', name: 'app_admin_commentaire_show', methods: ['GET'])]
    public function show(AdminCommentaire $adminCommentaire): Response
    {
        return $this->render('admin_commentaire/show.html.twig', [
            'admin_commentaire' => $adminCommentaire,
        ]);
    }

    #[Route('/{id}/edit', name: 'app_admin_commentaire_edit', methods: ['GET', 'POST'])]
    public function edit(Request $request, AdminCommentaire $adminCommentaire, EntityManagerInterface $entityManager): Response
    {
        $form = $this->createForm(AdminCommentaireType::class, $adminCommentaire);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->flush();

            return $this->redirectToRoute('app_admin_commentaire_index', [], Response::HTTP_SEE_OTHER);
        }

        return $this->render('admin_commentaire/edit.html.twig', [
            'admin_commentaire' => $adminCommentaire,
            'form' => $form,
        ]);
    }

    #[Route('/{id}', name: 'app_admin_commentaire_delete', methods: ['POST'])]
    public function delete(Request $request, AdminCommentaire $adminCommentaire, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete'.$adminCommentaire->getId(), $request->getPayload()->getString('_token'))) {
            $entityManager->remove($adminCommentaire);
            $entityManager->flush();
        }

        return $this->redirectToRoute('app_admin_commentaire_index', [], Response::HTTP_SEE_OTHER);
    }
}
